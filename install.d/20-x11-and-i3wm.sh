...
if dpkg -s xorg &>/dev/null; then
	log_info "xorg is already installed"
else
	sudo apt -y install \
		xorg openbox \
		compton \
...
if assert_is_installed i3; then
	log_info "i3 is already installed"
else
	sudo apt -y install \
		libxcb1-dev \
		libxcb-keysyms1-dev \
...
if assert_is_installed i3status-rs; then
	log_info "i3status-rs is already installed"
else
	cd $(mktemp -d)
